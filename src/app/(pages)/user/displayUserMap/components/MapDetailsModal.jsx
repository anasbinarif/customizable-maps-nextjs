...
import Image from "next/image"; // Import the new component
...
const MapDetailsModal = ({ open, onClose, map }) => {
    ...
    const getMarkerIcon = (color) => {
        if (!color || !window.google) return null; // Check if window.google is available
        return {
            ...
            strokeWeight: 1,
            scale: 2,
            anchor: new window.google.maps.Point(12, 24), // Use window.google
        };
    };

    const getMarkerColor = (tag) => {
        const filter = filters.find(filter => filter.name === tag);
        return filter ? filter.selectedColor : '#FF6347'; // Default color if tag not found
    };
            ...
            <Box sx={style}>
                ...
                <Box sx={contentStyle}>
                    ...
                    <Box sx={{ width: '60%', height: '100%' }}>
                        <LoadScript googleMapsApiKey={process.env.NEXT_PUBLIC_GOOGLE_PLACES_API_KEY}>
                            <GoogleMap
                                mapContainerStyle={mapContainerStyle}
                                center={{ lat: map.pinLatitude, lng: map.pinLongitude }}
                                zoom={13}
                                onLoad={(map) => (mapRef.current = map)}
                            >
                                {/* Pin Location */}
                                <Marker position={{ lat: map.pinLatitude, lng: map.pinLongitude }} />
